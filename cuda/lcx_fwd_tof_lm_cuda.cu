...
extern "C" void lcx_fwd_tof_lm_cuda(const float *h_xstart, 
                                         const float *h_xend, 
                                         float **d_img,
                                         ...
                                         int threadsperblock)
{
  ...
  float **d_G              = new float * [num_devices];
  int **d_index          = new int * [num_devices];
  ...
  for (int i_dev = 0; i_dev < num_devices; i_dev++) 
  {
    ...
    long long G_bytes_dev;
    long long index_bytes_dev;
    ...
    // calculate the number of bytes for the system Matrix G on the device
    G_bytes_dev = dev_nlors * h_img_dim[0] * 4 * sizeof(float);
    index_bytes_dev = dev_nlors * h_img_dim[0] * 4 * sizeof(int);
    ...
    error = cudaMalloc(&d_G[i_dev], G_bytes_dev);
    if (error != cudaSuccess){
        printf("cudaMalloc returned error %s (code %d), line(%d)\n", cudaGetErrorString(error), error, __LINE__);
        exit(EXIT_FAILURE);}
    cudaMemsetAsync(d_G[i_dev], 0, G_bytes_dev);

    error = cudaMalloc(&d_index[i_dev], index_bytes_dev);
    if (error != cudaSuccess){
        printf("cudaMalloc returned error %s (code %d), line(%d)\n", cudaGetErrorString(error), error, __LINE__);
        exit(EXIT_FAILURE);}
    cudaMemsetAsync(d_index[i_dev], 0, index_bytes_dev);
    ...
    // call the kernel
    lcx_fwd_tof_lm_cuda_kernel<<<grid,block>>>(d_xstart[i_dev], d_xend[i_dev], d_img[i_dev], 
                                                    d_img_origin[i_dev], d_voxsize[i_dev], 
                                                    d_p[i_dev], d_G[i_dev], d_index[i_dev],
    ...
    cudaMemcpyAsync(h_G + dev_offset * h_img_dim[0] * 4, d_G[i_dev], G_bytes_dev, cudaMemcpyDeviceToHost);
    cudaMemcpyAsync(h_index + dev_offset * h_img_dim[0] * 4, d_index[i_dev], index_bytes_dev, cudaMemcpyDeviceToHost);
    // unpin memory
