...
class PETSinogramParameters:
    ...
    def __init__(self,
        ...
        if span != 1:
            ...
            # TOF parameters
            self.ntofbins = ntofbins                                            # 17
            self.tofbin_width = tofbin_width                                    # 15
    ...
    def get_view_crystal_indices(self, views):
        """ get the trans-axial and axial crystal indices for a number of views of the sinogram
            ...
            these start and end detectors for the views.
        """
    ...
    # -------------------------------------------------------------------
    def lcx_sinogram_to_listmode(self,
                             sinogram,
                             return_multi_index=False):
        """ Convert an unsigned int sinogram to a list of events (list-mode data)
            ...
            -------
            If return_multi_index is False
              A 2D numpy int16 array of shape (nevents, 6) where each listmode event is characterized
              by 6 integer number (2 start crystals IDs, 2 end crystal IDs, tof bin, num of events in this tofbin)
            If return_multi_index is True
              a 2D array with the sinogram multi-index of every event is returned as well
              (2 output arguments)
        """

        if not np.issubdtype(sinogram.ravel()[0], np.signedinteger):    #.ravel()将数组拉成一维数组
            raise ValueError(
                'Sinogram must be of type unsigned int for conversion to LM.')
        ...
        events = np.zeros((np.sum(sinogram > 0), 6), dtype=np.int16)
        ...
        for x in it:
            if x > 0:
                events[counter, 0:2] = istart[it.multi_index[:-1]]
                events[counter, 2:4] = iend[it.multi_index[:-1]]
                
                events[counter, 4] = it.multi_index[-1] - self.ntofbins // 2

                events[counter, 5] = x

                if return_multi_index:
                    multi_index[counter, :] = it.multi_index
